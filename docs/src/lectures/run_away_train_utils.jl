...
function plot_train(sol, sys, tx = 0.0)
    ...
    scaling(t, p) = (10 + log((slt(t, p) / 1.0e5 + 0.01) / 100)) / 10
...
record_train(fig, tm; start = 0.72, stop = 0.729, step = 1.0e-4, framerate = 20, filename = "run_away_train_slow.mp4")
